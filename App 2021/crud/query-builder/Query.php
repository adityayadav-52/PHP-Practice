...
class Query extends DB{
	...
	$this->sql = $SQL;
	if($this->runQuery()){
		$this->last_pkid = mysqli_insert_id($this->getConnection());
		return true;
	}
	else{
		return false;
...
*/
echo PHP_EOL;
/*$query->insert('emp',['name'=>'shrivali1','email'=>'shrivali1@gmail.com']);
echo $query->getQuery();

echo PHP_EOL;*/


$records = $query->select('name','email')->table('emp')->where('id','>',1)->first();
print_r($records);
